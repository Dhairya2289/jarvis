...
from jarvis.config import BASE_DIR, TELEGRAM_TOKEN, ALLOWED_USER_IDS
...
def _on_window_open(class_name: str, title: str) -> None:
    ...
    if _should_trigger("anticipatory", 3600):
        try:
            from jarvis.anticipatory_engine import analyze_context_and_suggest
...
async def _scheduler() -> None:
    ...
    while True:
        ...
        if now.hour == 7 and now.minute >= 30 and morning_sent_date != today:
            ...
            try:
                from jarvis.planner import get_morning_briefing
        ...
        if now.weekday() == 5 and now.hour == 4 and failure_sent_date != today:
            ...
            try:
                from jarvis.failure_analyzer import analyze_failures
        ...
        if time.time() - life_state_ts > 1800:
            ...
            try:
                from jarvis.life_state_manager import update_life_state
                update_life_state()
            except Exception as e:
...
async def main() -> None:
    ...
    # Load evolved tools
    try:
        from jarvis.self_evolution import load_evolved_tools
        load_evolved_tools()
