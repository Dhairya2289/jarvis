...
def get_life_state() -> str:
    ...
    try:
        ...
        from jarvis.life_state_manager import get_identity_context
...
async def run_agent(
    ...
    """
    from jarvis.orchestrator import classify_task, record_outcome
    from jarvis.fast_path_router import execute_fast_path
    ...
    try:
        from jarvis.context_engine import classify_intent, route
    ...
    try:
        from jarvis.prompt_variants import get_random_variant, log_experiment
    ...
    # ── Session history ──────────────────────────────────
    try:
        from jarvis.session_memory import get_session_context, add_session_turn
    ...
    while iteration < max_iter:
        ...
        if not tool_uses:
            ...
            # ── Confidence check ──────────────────────────
            try:
                from jarvis.confidence import score as conf_score
        ...
        for tool in tool_uses:
            ...
            if str(output).startswith("[ERROR]") and iteration > 2:
                try:
                    from jarvis.self_evolution import attempt_self_evolution
    ...
    if success and final and "Error" not in final:
        try:
            from jarvis.knowledge_extractor import extract_and_add_to_graph
