...
from jarvis_v3.config import TELEGRAM_TOKEN, ALLOWED_USER_IDS, LOG_FILE, FCC_BASE_URL, FCC_AUTH_TOKEN
...
async def cmd_rate(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    from agent import rate_last_task
...
async def cmd_queue(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not task:
        from queue_manager import list_queue
        pending = list_queue()
        if not pending:
